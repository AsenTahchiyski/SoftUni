namespace _2.Scoreboard
{
    ...
    class Program
    {
        ...
        {
            string input;
            var scoreboard = new ScoreboardSlow();
            while ((input = Console.ReadLine()) != "End")
            {
                ...
                {
                    case "RegisterUser":
                        scoreboard.RegisterUser(parameters[1], parameters[2]);
                        break;
                    case "RegisterGame":
                        scoreboard.RegisterGame(parameters[1], parameters[2]);
                        break;
                    case "AddScore":
                        scoreboard.AddScore(parameters[1], parameters[2], 
                            parameters[3], parameters[4],
                            int.Parse(parameters[5]));
                        break;
                    case "ShowScoreboard":
                        scoreboard.ShowScoreboard(parameters[1]);
                        break;
                    case "ListGamesByPrefix":
                        scoreboard.ListGamesByPrefix(parameters[1]);
                        break;
                    case "DeleteGame":
                        scoreboard.DeleteGame(parameters[1], parameters[2]);
                        break;
                    default:
                        break;
                }
    ...
    public class ScoreboardSlow
    {
            ...
            string gameName, string gamePassword, int score)
        {
            ...
            this.scoresByGame[gameName].Values.Add(userName);
