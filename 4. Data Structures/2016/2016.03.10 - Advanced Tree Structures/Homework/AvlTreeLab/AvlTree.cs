// https://github.com/vdonchev/AdvancedTreeStructures-Homework/blob/master/01.AvlTree/AvlTree.cs
...
namespace AvlTreeLab
{
    ...
    public class AvlTree<T> where T : IComparable<T>
    {
        ...
        public IEnumerable<Node<T>> Range(T from, T to)
        {
            if (from.CompareTo(to) >= 0)
            {
                throw new ArgumentException("Invalid range bro.");
            }

            Node<T> startNode, endNode, currentNode;
            // find smallest node in range
            currentNode = this.root;
            while (currentNode != null && currentNode.Value.CompareTo(from) > 0)
            {
                if(currentNode.LeftChild != null && 
                    currentNode.LeftChild.Value.CompareTo(from) > 0)
                {
                    currentNode = currentNode.LeftChild;
                }
                else if(currentNode.RightChild != null && 
                    currentNode.RightChild.Value.CompareTo(from) > 0)
                {
            ...
            // find biggest node in range
            currentNode = this.root;
            while (currentNode != null && currentNode.Value.CompareTo(to) < 0)
            {
