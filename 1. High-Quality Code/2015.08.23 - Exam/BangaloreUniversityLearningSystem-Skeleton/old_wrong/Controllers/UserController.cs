namespace BangaloreUniversityLearningSystem.Controllers
{
    ...
    {
        public UserController(IData data, IUser user)
        {
            this.Data = data;
            this.User = user;
        }
        ...
        public IView Register(string username, string password, string confirmPassword, string role)
        {
            ...
            var user = new Models.User(username, password, userRole);
        ...
        public IView Login(string username, string password)
        {
            ...
            if (existingUser == null)
            {
                throw new ArgumentException(
                    string.Format("A user with username {0} does not exist.", username));
            }
