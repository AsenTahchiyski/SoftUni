        ...
        private static object[] MapParameters(IEndpoint executionEndpoint, MethodInfo action)
        {
            var parameters = action
                ...
                .Select<ParameterInfo, object>(p =>
                {
                    ...
                    {
                        return int.Parse(executionEndpoint.Parameters[p.Name]); // Name?
                    }
                    else if (p.ParameterType == typeof(DateTime))
                    {
                        var parameter = executionEndpoint.Parameters[p.Name];
                        return DateTime.ParseExact(parameter, Constants.DateFormat, CultureInfo.InvariantCulture);
                    }
                    else
                    {
