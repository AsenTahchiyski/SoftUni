...
namespace SolarSystem
{
    class OrbitsCalculator : INotifyPropertyChanged
    {
        ...
        // Performance tuning - step changed from decimal to double, cast to double in the loop removed
        // Performance tuning 2 - step increment changed from 0.00005 to 0.003 because the slowest rotation speed is 0.1 days per second. 
        // Having in mind that the human brain perceives object moving faster than 30 times per second as constant non-interrupted motion, the new increment step is 0.1 days per second / 30 frames per second = 0.00333, thus 0.003 would do just fine.
        private void EarthRotation()
        {
			for (double step = 0; step <= 360; step+=0.003)
			{
